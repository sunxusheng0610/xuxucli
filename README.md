...
xuxucli 是由 [xuxu](https://github.com/sunxusheng0610) 从零开发、逐步迭代并持续维护的 Java Agent CLI 项目，对标 Claude Code，从第一期的 `ReAct` 单代理循环逐步演进到第十六期的 `TUI 产品化`。
...
- 项目级记忆通过 `PAI.md` / `.xuxucli/PAI.md` 启动自动注入，适合提交到仓库的团队共享规则；`PAI.local.md` / `.xuxucli/PAI.local.md` 只做本地覆盖
...
- 配置持久化到 `~/.xuxucli/config.json`，API Key 可从配置、环境变量或 `.env` 读取
...
- 新增 `com.xuxucli.mcp` 模块，支持 stdio 子进程 server 与 Streamable HTTP 远程 server
- 启动时读取 `~/.xuxucli/mcp.json` 与 `.xuxucli/mcp.json`，项目级配置按 server 名覆盖用户级配置
...
- `~/.xuxucli/mcp.json` 不存在时会自动创建默认 chrome-devtools 配置；项目级 `.xuxucli/mcp.json` 仍可按 server 名覆盖
...
- `~/.xuxucli/mcp.json` 不存在时启动自动创建模板，默认使用 `--isolated=true` 临时浏览器 profile
...
- shared 模式下 `close_page` 只能关闭 xuxucli 自己创建的 tab；无法证明是 xuxucli 创建的 tab 会被策略层拒绝
...
- 三层加载位置（按优先级，后者整体覆盖同名 skill）：jar 内置 < 用户级 `~/.xuxucli/skills/<name>/` < 项目级 `<project>/.xuxucli/skills/<name>/`
- 启动期把启用 skill 的 `name` + `description` 注入三处 Agent 系统提示词索引段（启用上限 20 个，索引段 ≤ 4KB）
- 内置工具 `load_skill(name)`：LLM 在 system prompt 看到匹配 description 时主动调用，xuxucli 把 SKILL.md 正文（5KB 截断）写入 `SkillContextBuffer`，下一轮 user message 自动前置注入
...
- 启用状态持久化：`~/.xuxucli/skills.json` 的 `disabled` 列表，默认全启用
- 与 HITL 协同：Skill 内调用 `execute_command` 等危险工具仍走既有 HITL 审批，沿用 `execute_command` 工具维度全放行；不给 Skill 单独审批维度

设计意图：从「写工具」演进到「打包专家手册」。当工具堆成山（xuxucli 当前内置 9 个 + MCP 60+ 工具），用 Skill 给 LLM 一份按场景展开的"专家手册"，比往 system prompt 里塞更多规则更可扩展。
...
| 形态 | 启用方式 | 视觉风格 |
|---|---|---|
| **inline 流式 TUI**（默认） | 直接运行 / `XUXUCLI_RENDERER=inline` | Claude Code / Qoder 风格：π 主题彩色开屏、主屏直出、transcript 当前位置的 `* ` 输入提示、JLine `Status` 托管的底部 dock（YOLO/HITL、MCP、Skill、model、ctx、token、cwd 等关键字段带克制彩色高亮；ctx 是当前上下文估算，in/out/cache 是调用统计）、右侧输入提示、行内可折叠工具块（`Read 3 files (ctrl+o to expand)`）、行内 git diff、HITL 单字符 `[y/n/a/s/m]` 提示 |
| **lanterna 全屏 TUI** | `XUXUCLI_RENDERER=lanterna`（或兼容旧 `XUXUCLI_TUI=true`） | v16 三栏全屏：文件树 + 对话流 + 状态栏 + 底部输入栏，HITL 模态弹窗 |
| **plain 兜底** | `XUXUCLI_RENDERER=plain` | 纯 println，无折叠 / 状态栏，等价 v15 行为 |
...
- 对话历史保存到 `~/.xuxucli/history/session_*.jsonl`
- 兼容旧设置：`XUXUCLI_TUI=true` 自动映射为 `XUXUCLI_RENDERER=lanterna`（已 deprecated）
- `XUXUCLI_NO_STATUSBAR=true` 在 inline 模式下禁用 JLine 底部 dock（不适合 ANSI 光标控制的终端）
...
- 配置：`XUXUCLI_LSP_ENABLED=false` 可关闭，`XUXUCLI_LSP_MAX_DIAGNOSTICS=20` 可调整注入上限
...
- 快照仓库使用 JGit 纯 Java 实现，默认位于 `~/.xuxucli/snapshots/<project_hash>/<worktree_hash>/.git`，不写用户项目 `.git`
...
- 配置：`XUXUCLI_SNAPSHOT_ENABLED=false` 可关闭，`XUXUCLI_SNAPSHOT_MAX=50`、`XUXUCLI_SNAPSHOT_EXCLUDES=...`、`XUXUCLI_SNAPSHOT_DIR=...` 可调整策略
...
- 支持用户级覆盖 `~/.xuxucli/prompts/...`，支持项目级覆盖 `.xuxucli/prompts/...`，项目级优先级最高
- 覆盖是整文件替换；`base.md` 和最终 prompt 必须包含 `## Language`
- Prompt 改动审计模板见 `docs/prompt-analysis-template.md`

### 第二十期：异步后台任务 + Runtime API（MVP）

- `DurableTaskManager` 使用 SQLite 持久化后台任务队列，默认位置 `~/.xuxucli/tasks/tasks.db`
- 任务生命周期：`enqueued -> running -> completed / failed / canceled`
- `/task`、`/task add <任务内容>`、`/task cancel <task_id>`、`/task log <task_id>` 提供 CLI 闭环
- Worker Pool 默认 2 个后台 worker，可通过 `XUXUCLI_TASK_WORKERS` 调整
- `java -jar target/xuxucli-1.0-SNAPSHOT.jar serve --http --port 8080` 启动 localhost Runtime API
- Runtime API 端点：`POST /v1/threads`、`POST /v1/threads/{id}/turns`、`GET /v1/threads/{id}/events`
- Runtime API 强制要求 `XUXUCLI_RUNTIME_API_KEY` 或 `-Dxuxucli.runtime.api.key`
...
- 新增进程级入口：`xuxucli wechat setup`、`xuxucli wechat start`、`xuxucli wechat status`、`xuxucli wechat daemon start|stop|restart|status|logs`
- 新增交互式入口：在 xuxucli 主界面输入 `/wechat` 可扫码绑定并在当前进程后台启动微信通道；`/wechat setup` 重新扫码绑定，`/wechat status` 查看状态，`/wechat stop` 停止通道
...
- 微信侧用户消息会回显到 xuxucli 终端 transcript；xuxucli 终端继续显示 thinking / 工具调用过程，微信侧只接收 assistant 正文。iLink 协议层仍是 `text_item.text` 文本消息，没有显式 Markdown parse mode；xuxucli 会保留 ClawBot 稳定支持的 Markdown 子集（列表、引用、粗体、行内代码、真实代码块），把标题转成粗体标题、把表格转成移动端更稳的键值/列表，并过滤图片 Markdown / H5-H6 / 中文斜体等兼容性差的标记；非代码类 fenced block（流程说明、长中文箭头链）会解包并换行，避免微信侧出现横向滚动代码块。iLink 不提供真正 SSE 或改单条消息能力。
...
`com.xuxucli.policy` 包，作为 HITL 之外的辅助层（不是沙箱、不提供进程隔离）：
...
- `AuditLog` 结构化审计：危险工具调用按天写 JSONL 到 `~/.xuxucli/audit/`，含 `outcome (allow|deny|error)` 与 `approver (hitl|policy|none)`；`revert_turn` 也纳入危险工具链
- `write_file` 单文件 5MB 上限
- CLI 命令：`/policy` 查看安全策略状态、`/audit [N]` 看最近 N 条审计

**为什么不叫沙箱**：本地 Agent CLI（参考 Claude Code / Cursor / Aider）默认都不做容器/VM 沙箱——沙箱削弱 Agent 能力、给虚假安全感、体验更差。生产级 Agent 沙箱实际是 microVM-level（Devin / Modal / Anthropic Computer Use 用 Firecracker / gVisor）。xuxucli 的安全模型是 **HITL + 路径校验 + 命令快速拒绝 + 审计**，不是隔离。
...
- 📋 结构化审计：危险工具调用按天写一行 JSONL 到 `~/.xuxucli/audit/`，可通过 `/audit [N]` 查看
...
也可以在 xuxucli 内用命令写入 `~/.xuxucli/config.json`，不会覆盖 Kimi 配置：
...
长期记忆默认保存在用户目录下的 `~/.xuxucli/memory/long_term_memory.json`。
...
- `~/.xuxucli/PAI.md`：用户级稳定偏好，所有项目可见。
- `PAI.md` / `.xuxucli/PAI.md`：项目级团队规则，建议提交到 git。
- `PAI.local.md` / `.xuxucli/PAI.local.md`：本地覆盖，适合个人调试约定，建议加入 `.gitignore`。
- `@relative/path.md`：在 `PAI.md` 中导入项目根内的相对文件；越靠后的文件越接近本地覆盖，优先级越高。

可用 `/init` 为当前项目生成一份短 `PAI.md`。该命令默认不覆盖已有文件；确认需要重建时使用 `/init --force`。
代码索引默认保存在 `~/.xuxucli/rag/codebase.db`。
调试日志默认滚动写入 `~/.xuxucli/logs/xuxucli.log`，旧日志会按保留天数和总容量自动清理。
...
```bash
# 指定记忆目录
java -Dxuxucli.memory.dir=/tmp/xuxucli-memory -jar target/xuxucli-1.0-SNAPSHOT.jar

# 指定 RAG 索引目录
java -Dxuxucli.rag.dir=/tmp/xuxucli-rag -jar target/xuxucli-1.0-SNAPSHOT.jar

# 指定日志目录与保留策略
java -Dxuxucli.log.dir=/tmp/xuxucli-logs \
     -Dxuxucli.log.level=DEBUG \
     -Dxuxucli.log.maxHistory=3 \
     -Dxuxucli.log.maxFileSize=5MB \
     -Dxuxucli.log.totalSizeCap=20MB \
     -jar target/xuxucli-1.0-SNAPSHOT.jar
```

也可以放到 `.env` 或环境变量中：

```bash
XUXUCLI_LOG_LEVEL=DEBUG
XUXUCLI_LOG_DIR=/Users/yourname/.xuxucli/logs
XUXUCLI_LOG_MAX_HISTORY=7
XUXUCLI_LOG_MAX_FILE_SIZE=10MB
XUXUCLI_LOG_TOTAL_SIZE_CAP=100MB
```

### 2. 可选：配置 MCP server

MCP 子系统默认开启。`~/.xuxucli/mcp.json` 不存在时，xuxucli 会自动创建默认 chrome-devtools 配置：
...
需要继续接入其他 server 时，可编辑 `~/.xuxucli/mcp.json` 或项目内 `.xuxucli/mcp.json`：
...
`step_search` 是约定名称：如果项目 `.env`、用户 `~/.env` 或系统环境变量里存在 `STEP_API_KEY`，xuxucli 会自动内置这个远程 MCP；上面的手写配置只用于覆盖默认地址或自定义鉴权。当前模型为 `step-3.7-flash*` 时，内置 `web_search` / `web_fetch` 会优先代理到该 MCP server。
...
```bash
# macOS
open -na "Google Chrome" --args --remote-debugging-port=9222 --user-data-dir=/tmp/xuxucli-chrome-profile

# Windows
start chrome.exe --remote-debugging-port=9222 --user-data-dir=%TEMP%\xuxucli-chrome-profile

# Linux
google-chrome --remote-debugging-port=9222 --user-data-dir=/tmp/xuxucli-chrome-profile
```

通常不需要用户预先切换；Agent 如果遇到登录页会自己调用 `browser_connect`。手工调试时也可以在 xuxucli 内执行：
...
`/browser connect` 只在当前进程内把 `chrome-devtools` 切到 shared 模式，不会改写 `~/.xuxucli/mcp.json`。如果希望启动后默认 shared，可手动把 args 改为：
...
java -jar target/xuxucli-1.0-SNAPSHOT.jar
```

或者直接运行：

```bash
mvn clean compile exec:java -Dexec.mainClass="com.xuxucli.cli.Main"
```
...
同一轮模型返回多个工具调用时，xuxucli 会并行执行这些工具；如果工具之间有依赖关系，模型应分多轮调用。
...
- `xuxucli wechat setup` - 绑定微信 iLink 通道，选择 workspace 并完成扫码确认
- `xuxucli wechat start` - 前台启动微信通道
- `xuxucli wechat status` - 查看绑定状态和 daemon pid
- `xuxucli wechat daemon start|stop|restart|status|logs` - 管理本机微信通道后台进程

交互式斜杠命令：

- `/wechat` - 扫码绑定并启动微信 iLink 通道；已绑定时直接启动
- `/wechat setup` - 重新扫码绑定并启动微信通道
- `/wechat status` - 查看当前 xuxucli 进程内微信通道状态
- `/wechat stop` - 停止当前 xuxucli 进程内微信通道
...
- `/export` - 导出当前 ReAct 会话对话记录为 Markdown（包含完整 system prompt），写入 `~/.xuxucli/exports/session-*.md`
