...
- `src/main/java/com/xuxucli/lsp/LspManager.java`
  - 持有项目根路径
...
```bash
# 默认开启
XUXUCLI_LSP_ENABLED=true

# 默认 20
XUXUCLI_LSP_MAX_DIAGNOSTICS=20
```

系统属性同样支持：

```bash
-Dxuxucli.lsp.enabled=false
-Dxuxucli.lsp.max.diagnostics=20
```
...
- 不解析 `execute_command` 里的手写补丁，也不支持 `edit_file` / `apply_patch` 工具，因为当前 xuxucli 内置工具里还没有这两个工具。
