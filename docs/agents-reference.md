...
1. `~/.xuxucli/config.json` 中对应 provider 的 `apiKey`
...
| 数据 | 默认路径 | 覆盖方式 |
|------|----------|----------|
| 长期记忆 | `~/.xuxucli/memory/long_term_memory.json` | `-Dxuxucli.memory.dir` |
| 项目级记忆 | `PAI.md` / `.xuxucli/PAI.md` / `PAI.local.md` | 用户级稳定偏好：`~/.xuxucli/PAI.md` |
| RAG 索引 | `~/.xuxucli/rag/codebase.db` | `-Dxuxucli.rag.dir` |
| 审计日志 | `~/.xuxucli/audit/audit-YYYY-MM-DD.jsonl` | `XUXUCLI_AUDIT_DIR` / `-Dxuxucli.audit.dir` |
| Side-Git 快照 | `~/.xuxucli/snapshots/<project_hash>/<worktree_hash>/.git` | `XUXUCLI_SNAPSHOT_DIR` / `-Dxuxucli.snapshot.dir` |
| 后台任务 | `~/.xuxucli/tasks/tasks.db` | — |

### Snapshot Config

系统属性 > 环境变量 > 默认值：`xuxucli.snapshot.enabled`(true) / `xuxucli.snapshot.max`(50) / `xuxucli.snapshot.excludes`(.git,.xuxucli/snapshots,target,node_modules,dist,.idea,*.class,*.jar) / `xuxucli.snapshot.dir`(~/.xuxucli/snapshots)
...
系统属性 > 环境变量/.env > 默认值：`XUXUCLI_LOG_DIR`(~/.xuxucli/logs) / `XUXUCLI_LOG_LEVEL`(INFO) / `XUXUCLI_LOG_MAX_HISTORY`(7) / `XUXUCLI_LOG_MAX_FILE_SIZE`(10MB) / `XUXUCLI_LOG_TOTAL_SIZE_CAP`(100MB)

### ReAct/SubAgent Budget Config

系统属性 > 默认值：`xuxucli.react.token.budget`(Integer.MAX_VALUE) / `xuxucli.react.stagnation.window`(3) / `xuxucli.react.hard.max.iterations`(50)
...
系统属性 > 默认值：`xuxucli.llm.connect.timeout.seconds`(60) / `xuxucli.llm.read.timeout.seconds`(300) / `xuxucli.llm.write.timeout.seconds`(60) / `xuxucli.llm.call.timeout.seconds`(600)
...
1. 用户级：`~/.xuxucli/mcp.json`
2. 项目级：`.xuxucli/mcp.json`
...
- 加载顺序：`~/.xuxucli/PAI.md` → `PAI.md` → `.xuxucli/PAI.md` → `PAI.local.md` → `.xuxucli/PAI.local.md`
...
- shared 模式 close_page 只允许关闭 xuxucli 创建的 tab

### Skill System

- 三层加载：jar 内置 < 用户级 ~/.xuxucli/skills/ < 项目级 .xuxucli/skills/
...
- 环境变量：`XUXUCLI_RENDERER=inline|lanterna|plain`
- `XUXUCLI_TUI=true`(旧) → lanterna + deprecation 提示
- `XUXUCLI_NO_STATUSBAR=true`：禁用底部状态栏
...
- `/export` 导出当前 ReAct conversationHistory 为 Markdown 到 `~/.xuxucli/exports/session-*.md`；包含完整 system prompt，便于检查 LLM 实际接收前的指令，命令不接受路径参数。
...
- `XUXUCLI_LSP_ENABLED=false` 关闭

### Git Side-History Snapshot (Phase 18)

- side-git 在 ~/.xuxucli/snapshots/ 维护独立仓库（JGit，不依赖系统 git）
...
- 覆盖优先级：jar 内置 < 用户级 ~/.xuxucli/prompts/ < 项目级 .xuxucli/prompts/
...
- XfyunMaaSClient：Qwen3.6-35B-A3B，默认 https://maas-api.cn-huabei-1.xf-yun.com/v2，OpenAI-compatible 讯飞星辰 MaaS；可用 `/config provider xfyun ...` 写入配置后 `/model xfyun` 切换。`model` 必须使用 MaaS 服务管控页展示的 modelId；微调模型可配置 `--lora-id <resourceId>`，作为 HTTP header `lora_id` 发出；该 provider 不发送 xuxucli 内置 tools。
...
# XUXUCLI_SNAPSHOT_ENABLED=true
# XUXUCLI_SNAPSHOT_MAX=50
# XUXUCLI_SNAPSHOT_EXCLUDES=.git,.xuxucli/snapshots,target,node_modules,dist,.idea,*.class,*.jar
# XUXUCLI_SNAPSHOT_DIR=/Users/yourname/.xuxucli/snapshots
# XUXUCLI_TUI=true
# NO_TUI=true
```
