 ...
 * 1. 系统属性：{@code xuxucli.react.token.budget} / {@code xuxucli.react.stagnation.window} /
 *    {@code xuxucli.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dxuxucli.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dxuxucli.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("xuxucli.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("xuxucli.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("xuxucli.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
