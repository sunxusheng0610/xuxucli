package com.xuxucli.agent;

import com.xuxucli.llm.LlmClient;
import com.xuxucli.llm.LlmTraceLogger;
import com.xuxucli.context.ContextProfile;
import com.xuxucli.context.TokenUsageFormatter;
import com.xuxucli.lsp.LspDiagnosticReport;
import com.xuxucli.memory.ConversationHistoryCompactor;
import com.xuxucli.memory.ExplicitMemoryHints;
import com.xuxucli.memory.MemoryManager;
import com.xuxucli.prompt.PromptAssembler;
import com.xuxucli.prompt.PromptContext;
import com.xuxucli.prompt.PromptMode;
import com.xuxucli.prompt.ProjectMemoryLoader;
import com.xuxucli.render.PlainRenderer;
...
import com.xuxucli.skill.SkillIndexFormatter;
import com.xuxucli.skill.SkillRegistry;
import com.xuxucli.util.AnsiStyle;
import com.xuxucli.tool.ToolRegistry;
import com.xuxucli.tool.ToolRegistry.ToolExecutionResult;
import com.xuxucli.tool.ToolRegistry.ToolInvocation;
import com.xuxucli.util.TerminalMarkdownRenderer;
import com.xuxucli.image.ImageReferenceParser;
...
public class Agent {
    ...
    public String getContextStatus() {
        com.xuxucli.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.xuxucli.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.xuxucli.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.xuxucli.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.xuxucli.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.xuxucli.memory.MemoryEntry.estimateTokens(
