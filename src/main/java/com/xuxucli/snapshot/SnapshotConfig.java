...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("xuxucli.snapshot.enabled", "XUXUCLI_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("xuxucli.snapshot.dir", "XUXUCLI_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".xuxucli", "snapshots").toString()));
        int max = readInt("xuxucli.snapshot.max", "XUXUCLI_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("xuxucli.snapshot.excludes", "XUXUCLI_SNAPSHOT_EXCLUDES", ""));
